...
//        utilityRetrieveOperator = Procedural.utilityRetrieveOperatorDefault
    ...
    */
    func fireProduction(_ inst: Instantiation, compile: Bool) -> (Bool, Prim?, Double) {
        if compile {
    ...
    func findMatchingProduction() -> Instantiation {
        ...
        if best == nil || best!.u < primU {
            if condition != nil {
                ...
                let p = Production(name: "t" + primName, model: model, condition: condition!.description, action: action==nil ? nil : action!.description,  parent1: nil, parent2: nil, taskID: 0, u: primU)
                ...
                let p = Production(name: "t" + primName, model: model, condition: nil, action: action!.description,  parent1: nil, parent2: nil, taskID: 0, u: primU)
                let prim = Prim(name: primName, model: model)
                p.addAction(prim)
                return Instantiation(prod: p, time: model.time, u: primU)
            }
    ...
    func compileProductions(_ p1: Production, inst2: Instantiation) {
       ...
       // let nameP1 = p1.name.hasPrefix("t") ? p1.name.substring(from: p1.name.characters.index(p1.name.startIndex, offsetBy: 1)) : p1.name
        ...
        if let existingP = productions[newFullName] {
            ...
            let newP = Production(name: newName, model: model, condition: p1.condition, action: p1.action, parent1: p1, parent2: p2, taskID: model.currentTaskIndex!, u: defaultU)
