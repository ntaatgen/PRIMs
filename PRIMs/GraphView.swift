...
class GraphView: NSView {
    var lineWidth: CGFloat = 3 { didSet { setNeedsDisplayInRect(self.frame) }}
    var orgX: CGFloat {
        get {
...
//            println("x = \(xp) y = \(yp)")
            if first {
                path.moveToPoint(NSPoint(x: xp, y: yp))
                first = false
            } else {
                path.lineToPoint(NSPoint(x: xp, y: yp))
            }
        }
